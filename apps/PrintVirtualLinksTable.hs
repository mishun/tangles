...
main = do
    ...
    let diagrams =
            filter (\ link -> not (isReidemeisterReducible link) && testPrime link) $
                tangleStarGlue
                    (\ n -> filter ((== targetGenus) . genusOfChordDiagram . fst) $ listChordDiagrams $ generateNonPlanarRaw n)
    ...
    when (length (collisionClasses sifted) > 0) $ do
        renderSVG (printf "virtual-links-collisions-%i-%i.svg" targetGenus maxN) (Width 500) $ pad 1.05 $
