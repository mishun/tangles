...
test = testGroup "Dotted cobordisms"
    [ generalCobordismTests (undefined :: DottedCobordism Integer)
    , generalCannedCobordismTests (undefined :: DottedCobordism Integer)
{-
    , testCase "Reidemeister I homotopy" $ do
        let d = horizontalComposition 2 (saddleCobordism, 0) (planarPropagator 1, 0) :: Cob
            h = planarPropagator 1 ⊗ capCobordism :: Cob
        d @?= h
        return ()-}
    ]
