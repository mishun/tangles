module Math.Topology.KnotTh.Invariants.Skein.StateModels.PlanarDiagramsSum
    ( PlanarDiagramsSum
    ) where
...
instance Functor PlanarChordDiagram where
    fmap f (PlanarChordDiagram p x) = PlanarChordDiagram p $! f x


instance (NFData a) => NFData (PlanarChordDiagram a) where
    rnf (PlanarChordDiagram p x) = p `seq` rnf x
...
data PlanarDiagramsSum a = PlanarDiagramsSum !Int ![PlanarChordDiagram a] deriving (Eq, Ord)
...
singletonStateSum :: PlanarChordDiagram a -> PlanarDiagramsSum a
...
concatStateSums list @ (PlanarDiagramsSum order _ : _) =
    PlanarDiagramsSum order $ map (\ (!k, !v) -> PlanarChordDiagram k v) $
...
mapStateSum :: (Eq a, Num a) => (PlanarChordDiagram a -> PlanarDiagramsSum a) -> PlanarDiagramsSum a -> PlanarDiagramsSum a
...
forAllSummands :: (Monad m) => PlanarDiagramsSum a -> (PlanarChordDiagram a -> m ()) -> m ()
        ...
        concatStateSums . map (\ (skein, factor) ->
                let a = listArray (0, 3) $
                        case skein of
                            ...
                            Lzero  -> [3, 2, 1, 0]
                            Linfty -> [1, 0, 3, 2]
                in singletonStateSum $ PlanarChordDiagram a factor
            )

    asConst _ (PlanarDiagramsSum _ []) = 0
    asConst _ (PlanarDiagramsSum _ [PlanarChordDiagram _ x]) = x
