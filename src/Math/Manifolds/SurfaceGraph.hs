...
constructFromArray g =
	constructFromList $! map (map (\ (i, j) ->
		let 	is = if inRange (bounds g) i then index (bounds g) i else error "constructFromArray: vertex index is out of bound"
			ja = g ! i
			js = if inRange (bounds ja) j then index (bounds ja) j else error "constructFromArray: dart index is out of bound"
		in (is, js)
	) . elems) $! elems g
